...
class AscendAttnBackend(AttentionBackend):
    ...
    def forward_extend(
        ...
        if not self.use_mla:
            ...
            """
            k_cache [T_cache, tp_k_head_num, qk_head_dim]
            v_cache [T_cache, tp_v_head_num, v_head_dim]
            使用page attention时 T_cache = num_blocks * page_size
            """
            ...
            if self.use_fia:
                ...
                q_len_offset = 0
                """
                当前 CANN 的 FIA npu_fused_infer_attention_score 在这个用法下不支持一次处理 multi-batch/多个 request
                而 forward_batch 的 extend/prefill 通常是把多个序列的 token 拼成一个大扁平序列concatenate
                extend_seq_lens_cpu 就是每个 request 的 q_len
                所以只能用 q_len_offset 切片, 把每个 request 单独喂给算子
                
                q/k/v 都是“按 request 拼起来的扁平序列
                (因为是prefill阶段prompt self-attention,所以q/k/v长度相同)
                """
                ...
                if layer.qk_head_dim <= 128:
                    ...
                    """
                    对一个 batch 里的多条变长序列 做 prefill/extend 注意力
                    Q 来自本次新增 tokens,K/V 来自 paged KV cache
                    用 block_table 把每条序列的 K/V 从 cache 里 gather 出来
                    再做 FlashAttention 式的 fused softmax+matmul
                    """
            ...
            """
            extend_prefix_lens_cpu 是这次 extend 的每条序列的 prefix历史缓存长度
            sum(...) > 0 说明至少有一条序列不是“从 0 开始的纯 prefill”,而是“在已有上下文上继续 extend”
            下面的实现分成 3 步：
            1.先算“新增 tokens 之间”的注意力,拿到 attn_output 和 attn_lse
            2.再从 KV cache 里把历史块加载出来构造历史侧的 k_nope/v
            3.再把历史 KV 融合进去,用 pre_out/prev_lse 做稳定拼接
            """
            ...
            """
            MLA Prefill如果不满足 sum(extend_prefix_lens_cpu) > 0 则必走FIA分支
            """
            assert (
    ...
    def forward_decode_graph(
        ...
        if not self.use_mla:
            ...
            """
            非 MLA 分支 num_tokens < get_attention_tp_size()
            走 npu_fused_infer_attention_score.out(带 workspace)
            当前 CANN 的 paged attention(PA)内核对 batch siz/不支持
            该版本显式预取 workspace
            graph capture/replay 希望不要在图里动态申请内存
            npu_fused_infer_attention_score.out使用out参数把输出写进已经分配好的tensor而非返回结果

            num_tokens >= tp_size 时直接走_npu_paged_attention
            ...
            """
            FIA 时需要区分FIA_NZ和普通FIA
            只用npu_fused_infer_attention_score
            因为paged_attention算子不支持graph
            """
            c_kv, k_rope = forward_batch.token_to_kv_pool.get_kv_buffer(layer.layer_id)
            if is_fia_nz():
                k_rope_cache = _reshape_kv_for_fia_nz(
    ...
    def forward_decode(
        ...
        if not self.use_mla:
            ...
            if self.use_fia:
                ...
                """
                传了 block_size 和 block_table
                内部用Paged Attention 逻辑来处理 KV cache
                而之前forward_extend里就没有
                """
                attn_output, _ = torch.ops.npu.npu_fused_infer_attention_score(
                    q.view(
            ...
            if self.use_fia and (layer.tp_q_head_num // layer.tp_k_head_num) >= 8:
                ...
                """
                is_fia_nz时mla_preprocess 负责把数据写进 cache 时按 NZ 友好方式组织
                即is_fia_nz时kv_c和k_pe底层已经是NZ方式排布了
                decode 这里的 _reshape_kv_for_fia_nz 负责把这块已经按 NZ 写好的内存的逻辑shape变成NZ
                """
                if is_fia_nz():
                ...
                """
                MLA 在deocde时 如不开启FIA 走paged_attention
                _npu_paged_attention_mla 不支持graph
                """
                assert (
                    self.graph_mode == False
